...
public class TitleFetcher {
	...
	public static void fetchTitles(String inputdir, int numThreads)
		...
		for (File file : directory) {
			...
			if (file.getName().matches("tweets.*.json")) {
				...
				for (String line; (line = jsonReader.readLine()) != null;) { // too
																				// lazy
																				// to
																				// catch
																				// this
				...
				while (!inputJsonQueue.isEmpty()) {
					...
					if (!tweet.containsKey("linkTitle")
							&& !tweet.containsKey("hasBadLink")) { // only
																	// consider
																	// new
																	// tweets
						String text = tweet.get("text").toString();
						String maybeUrlString = getUrlString(text); // read
																	// maybe as
																	// "could be null or..."
				...
				BufferedWriter jsonWriter = new BufferedWriter(new FileWriter(
						file, false));
		...
		System.out.println("Processed " + Integer.toString(tweetCount)
				+ " tweets across " + Integer.toString(fileCount) + " files");
		System.out.println("Retrieved titles for "
				+ Integer.toString(titleCount) + " out of "
				+ Integer.toString(matchCount) + " links");
	...
	public static String getTitle(String urlString) {
		String title = null;
		try {
			Document doc = Jsoup.connect(urlString).timeout(crawlTimeoutMs)
					.get();
			title = doc.title();
		} catch (IOException e) {
			title = "";
	...
	public static String getUrlString(String text) {
		...
		for (String word : words)
			try {
				URL url = new URL(word); // uses the built in parsing abilities
